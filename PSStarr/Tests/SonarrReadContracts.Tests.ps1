...
InModuleScope PSStarr {
    Describe '<Command> Sonarr route contract' -ForEach @(
    ...
    Describe 'Sonarr read connection contract' {
            BeforeEach {
                Mock Invoke-StarrApiRequest { [pscustomobject]@{ marker = 'fixture' } }
            }
    
            It 'supports explicit and inferred connections' {
                Get-StarrSonarrParse -Url 'http://localhost:8989/base' -ApiKey fixture-key -Title Fixture
    
                Should -Invoke Invoke-StarrApiRequest -Times 1 -Exactly -ParameterFilter {
                    $ExpectedApplication -eq 'Sonarr' -and $Url -eq 'http://localhost:8989/base' -and $ApiKey -eq 'fixture-key'
                }
    
                Get-StarrSonarrParse -Title Fixture
            ...
            It 'validates credentials before transport' {
                { Get-StarrSonarrParse -Title Fixture -InstanceName ' ' } | Should -Throw
                { Get-StarrSonarrParse -Title Fixture -Url 'ftp://localhost' -ApiKey fixture-key } | Should -Throw
                { Get-StarrSonarrParse -Title Fixture -Url 'http://localhost:8989' -ApiKey ' ' } | Should -Throw
